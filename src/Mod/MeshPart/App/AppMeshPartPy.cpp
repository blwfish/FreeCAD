...
private:
    ...
    // Keyword-argument methods (e.g. meshFromShape) dispatch through this path.
    // Must mirror invoke_method_varargs exception handling — without this override
    // any C++ exception (e.g. BRepMesh_IncrementalMesh throwing on bad geometry)
    // propagates uncaught and calls std::terminate, crashing FreeCAD.
    // See: https://github.com/FreeCAD/FreeCAD/issues/27752
    Py::Object invoke_method_keyword(void *method_def,
                                     const Py::Tuple &args,
                                     const Py::Dict &kwds) override
    {
        try {
            return Py::ExtensionModule<Module>::invoke_method_keyword(method_def, args, kwds);
        }
        catch (const Standard_Failure &e) {
            std::string str;
            Standard_CString msg = e.GetMessageString();
